...
console.log(
  "1. Checking quizData:",
  typeof quizData !== "undefined" ? "Found!" : "NOT FOUND!",
);
const questions =
  typeof quizData !== "undefined" ? quizData[currentSubject] : [];
...
document.getElementById("subject-icon").src =
  subjectIcons[currentSubject] || subjectIcons["HTML"];
...
function renderQuestion() {
  ...
  if (!data.options) {
    optionsContainer.innerHTML =
      "<p class='text-white'>Error: No options found in your data file.</p>";
    return;
  }
  ...
  data.options.forEach((option, i) => {
    ...
    btn.className =
      "option-btn group w-full flex items-center gap-4 bg-[#3b4d66] p-4 md:p-5 rounded-2xl md:rounded-[24px] border-[3px] border-transparent transition-all shadow-sm text-left outline-none";
...
function applyStatus(btn, box, colorHex, iconPath) {
  ...
  btn.querySelector(".icon-container").innerHTML =
    `<img src="${iconPath}" class="w-8 h-8 md:w-10 md:h-10">`;
}
